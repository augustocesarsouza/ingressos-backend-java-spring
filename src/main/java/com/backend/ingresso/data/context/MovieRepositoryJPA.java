...
public interface MovieRepositoryJPA extends JpaRepository<Movie, UUID> {
    @Query("SELECT new package com.backend.ingresso.application.dto.MovieDTO(m.Id, m.Title) FROM Movie AS m WHERE m.Id = :movieId")
    ...
    @Query("SELECT m FROM Movie AS m WHERE m.Id = :movieId")
    MovieDTO getMovieById_Info_All(UUID movieId);
    //testar esse 'setDataForGetByRegionId' pode não funcionar
    @Query("SELECT new com.backend.ingresso.application.dto.MovieDTO().setDataForGetByRegionId(m.Id, m.Title, m.ImgUrl, m.MovieRating) " +
